...
program define dmlmed_type2, rclass
	...
	forval k=1/`xfits' {
		...
		qui rforest `dvar' `mvar' `cvars' if `kpart'!=`k' & `touse', ///
			type(class) iter(`ntrees') lsize(`lsize') seed(`seed')
		...
		di "   Training random forest for mu`d'(C,M)"
		...
		di "   Training random forest for mu`dstar'(C,M)"
